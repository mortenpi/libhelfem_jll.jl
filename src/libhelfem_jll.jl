...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import libhelfem_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("libhelfem_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "libhelfem_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"libhelfem", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "libhelfem_jll", "src", "libhelfem_jll.jl"))
        @info("libhelfem_ll dev'ed out to /home/mortenpi/.julia/dev/libhelfem_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("e795dbb4-5d3b-534d-89bd-7ff5a0f64a00"))
platforms = [Pkg.Artifacts.unpack_platform(e, "libhelfem", artifacts_toml) for e in artifacts["libhelfem"]]
...
if best_platform === nothing
    @debug("Unable to load libhelfem; unsupported platform $(triplet(platform_key_abi()))")
else
